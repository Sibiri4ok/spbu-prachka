...
def slots_kb(date: str, booked_slots: List, user_slot: int):
    ...
    for slot_num in range(1, 21):
        if slot_num in booked_slots and slot_num == user_slot:
            ...
            builder.button(text=f"❌", callback_data=f"close_slot")
        else:
...
def slots_admin_kb(date: str, booked_slots: List):
    slots = {slots[2]: (slots[0], slots[1]) for slots in booked_slots}
    builder = InlineKeyboardBuilder()
    for slot_num in range(1, 21):
...
def cancel_slot(date: str, user_slot: int):
    button1 = InlineKeyboardButton(
        text=f"Вернуться назад", callback_data=f"back_to_slots_{date}_{user_slot}"
    )
    button2 = InlineKeyboardButton(
        text="Отменить слот", callback_data=f"cancel_slot_{date}_{user_slot}"
    )
...
def admin_close_options_slot(date: str, user_slot: int):
    ...
    button1 = InlineKeyboardButton(
        text=f"Вернуться назад", callback_data=f"admin_date_{date}"
    )
    button2 = InlineKeyboardButton(
        text="Удалить слот", callback_data=f"admin_delete_slot_{date}_{user_slot}"
    )
    return InlineKeyboardMarkup(inline_keyboard=[[button1], [button2]])
...
def admin_available_options_slot(date: str, user_slot: int):
    ...
    button2 = InlineKeyboardButton(
        text="Добавить студента", callback_data=f"admin_add_slot_{date}_{user_slot}"
    )
    return InlineKeyboardMarkup(inline_keyboard=[[button1], [button2]])
...
def add_remove_info():
    button1 = InlineKeyboardButton(
        text=f"Добавить сообщение", callback_data=f"add_info"
    )
    button2 = InlineKeyboardButton(
        text="Удалить сообщение", callback_data=f"remove_info"
    )
    return InlineKeyboardMarkup(inline_keyboard=[[button1], [button2]])
